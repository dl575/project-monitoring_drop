...
for filename in glob.glob(os.path.join(compile_dir, "malarden*.c")):
  ...
  for wcet in wcets:
    ...
    compile_cmd = "arm-linux-gnueabi-gcc -DUNIX -DWCETC_FAC=%d -DWCETC_IS=%d\
        -DWCETC_FC=%d malarden_%s.c ../include/malarden.c -o malarden_%s.arm \
        --static; mv malarden_%s.arm ../multi_malarden.arm" % \
        (wcet, wcet, wcet, benchmarks, benchmarks, benchmarks)
