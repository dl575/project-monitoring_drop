...
vec_mpy1(short y[], const short x[], short scaler)
{
	...
	for (i = 0; i < 150; i++){
        ...
        ENDSTART_SUBTASK(WCET_EDN_2);
        
		y[i] += ((scaler * x[i]) >> 15);
    }
    
    ENDSTART_SUBTASK(WCET_EDN_3);
}
...
mac(const short *a, const short *b, long int sqr, long int *sum)
{
	...
	for (i = 0; i < 150; i++) {
        ...
        ENDSTART_SUBTASK(WCET_EDN_4);
        
		dotp += b[i] * a[i];
		sqr += b[i] * b[i];
	}
    
    ENDSTART_SUBTASK(WCET_EDN_5);
...
latsynth(short b[], const short k[], long int n, long int f)
{
	...
	for (i = n - 2; i >= 0; i--) {
        ...
        ENDSTART_SUBTASK(WCET_EDN_10);
	
        f -= b[i] * k[i];
		b[i + 1] = b[i] + ((k[i] * (f >> 16)) >> 16);
	}
    
    ENDSTART_SUBTASK(WCET_EDN_11);
