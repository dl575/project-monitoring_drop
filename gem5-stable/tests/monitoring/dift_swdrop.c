...
int main(int argc, char *argv[]) {
  ...
  int threshold = MON_WCET - MON_DROP_WCET;
...
//        bool settag = READ_FIFO_SETTAG;
    ...
    // Not enough slack
      ...
      // Store instruction
      if (temp = READ_FIFO_STORE) {
        // Clear the target tag in memory
        tagmem[(READ_FIFO_MEMADDR >> 2) % METADATA_ADDRESSES] = false;
      // Indirect control instruction
      } else if (temp = READ_FIFO_INDCTRL) {
        // Check, do nothing
      // Load or Integer ALU instruction
      } else {
        // Clear the target register tag
        unsigned rd = READ_FIFO_RD;
        tagrf[rd] = false;
      }
    } // READ_SLACK_DROP
